...
async def game_knb_info(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        '<i>Готовы сделать ставку?</i>',
        reply_markup=InlineKeyboardBuilder([
            [InlineKeyboardButton(text="🪨 Сделать ставку", callback_data="game_knb")],
            [InlineKeyboardButton(text="📊 Меню игр", callback_data="back_to_games")]
        ]).adjust(1).as_markup()
    )
...
async def game_knb_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f'<i>Минимальная ставка: 0.1$</i>',
        reply_markup=InlineKeyboardBuilder([
            [InlineKeyboardButton(text="❌ Назад", callback_data="game_knb_info")]
        ]).as_markup()
    )
    ...
    try:
        ...
        await message.answer(
            ...
            reply_markup=InlineKeyboardBuilder([
                ...
                [InlineKeyboardButton(text="❌ Отмена", callback_data="game_knb_info")]
            ]).adjust(2).as_markup()
        )
...
async def process_knb_game(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.answer(
            ...
            f'💰 <b>Новый баланс:</b> {new_balance}$',
            reply_markup=InlineKeyboardBuilder([
                [InlineKeyboardButton(text="🪨 Сыграть еще", callback_data="game_knb_info")],
                [InlineKeyboardButton(text="📊 Меню игр", callback_data="back_to_games")]
            ]).adjust(1).as_markup()
        )
        await state.clear()
